...
After discussing the theoretical framework of our analysis (\sectref{sec:prandi:2}), we present our research desiderata and the structure of an exploratory study conducted to test our research methodology (\sectref{sec:prandi:3}). \sectref{sec:prandi:4} describes the rationale behind the stimuli used in the experiment and the features of the speeches used. We then present the results of the analysis of the transcriptions (\sectref{sec:prandi:5}), which we use to evaluate the appropriateness of our stimuli. In the conclusions, we address future work and provide suggestions for further research.
...
\&  \&  \& {vector} \& {$\emptyset$} \& {$\emptyset$} \& {$\emptyset$} \& {1} \& {$\emptyset$} \& {1} \& {$\emptyset$} \& {$\emptyset$}\\
...
\foreach \i in {8,10,12} \node [fit=(prandifig3-\i-3)(prandifig3-\i-12), fill=black!12,minimum height=1.4cm,yshift=-.5ex] {};
\foreach \j in {8,10} \node [fit=(prandifig3-4-\j)(prandifig3-12-\j), fill=black!12,minimum width=1.26cm,inner xsep=0pt] {};
\foreach \x/\y in {8/8,8/10,10/8,10/10,12/8,12/10} \node [fit=(prandifig3-\x-\y), fill=black!50,minimum width=1.5cm,minimum height=1.4cm,yshift=-.75ex] {};
\end{scope}
\draw (prandifig3-4-12.south east) -| (prandifig3-12-4.south west);
\draw (prandifig3-3-12.south east) -| (prandifig3-12-3.south west);
\draw (prandifig3-3-11.north west) -- (prandifig3-3-12.north east);
\draw (prandifig3-3-9.north west) -- (prandifig3-3-10.north east);
\draw (prandifig3-3-5.north west) -- (prandifig3-3-8.north east);
\draw (prandifig3-11-3.north east) -- (prandifig3-12-3.north west);
\draw (prandifig3-9-3.north east) -- (prandifig3-10-3.north west);
\draw (prandifig3-5-3.north east) -- (prandifig3-8-3.north west);
\end{tikzpicture}
}
...
\end{tabular}
\end{center}
\caption{Conflict matrix for simultaneous interpreting (adapted from \citealt[1385]{Seeber2007})\label{fig:prandi:3}\todo[inline]{ToDo for Langsci: Spacing between rows and columns, better rules!}}
\end{figure}
...
\&  \&  \&  \&  \& ~ \&  \&  \&  \&  \&  \&  \\  %\multicolumn{8}{c}{listening \\& comprehension}\\
\&  \&  \&  \&  \& ~ \&  \&  \&  \&  \&  \&  \\  % \multicolumn{4}{c}{perceptual} \& \multicolumn{2}{c}{cognitive} \& \multicolumn{2}{c}{response}\\
\&  \&  \& {vector} \& {$\emptyset$} \& {$\emptyset$} \& {$\emptyset$} \& {1} \& {$\emptyset$} \& {1} \& {$\emptyset$} \& \node [hea] (prandifig7-3-12) {$\emptyset$};\\
...
\begin{scope}[on background layer]
\foreach \i in {5,6,8,10,11,12} \node [fit=(prandifig7-\i-3)(prandifig7-\i-12), fill=black!12,minimum height=1.4cm,yshift=-.5ex,inner ysep=0pt] {};
\foreach \j in {8,10} \node [fit=(prandifig7-4-\j)(prandifig7-12-\j), fill=black!12,minimum width=1.26cm,inner xsep=0pt] {};
\foreach \x in {5,6,8,10,11,12} {
    \foreach \y in {8,10} \node [fit=(prandifig7-\x-\y), fill=black!50,minimum width=1.5cm,minimum height=1.4cm,yshift=-.75ex] {};
    }
\end{scope}
\draw (prandifig7-4-12.south east) -| (prandifig7-12-4.south west);
\draw (prandifig7-3-12.south east) -| (prandifig7-12-3.south west);
\draw (prandifig7-3-11.north west) -- (prandifig7-3-12.north east);
\draw (prandifig7-3-9.north west) -- (prandifig7-3-10.north east);
\draw (prandifig7-3-5.north west) -- (prandifig7-3-8.north east);
\draw (prandifig7-11-3.north east) -- (prandifig7-12-3.north west);
\draw (prandifig7-9-3.north east) -- (prandifig7-10-3.north west);
\draw (prandifig7-5-3.north east) -- (prandifig7-8-3.north west);
\end{tikzpicture}}
\caption{Conflict matrix for \textsc{si} with spreadsheet (Excel) TIC = 16.8\label{fig:prandi:7}\todo[inline]{Todo for LangSci: Spacing between rows and columns, better rules!}}
\end{figure}
...
\begin{scope}[on background layer]
\foreach \i in {8,10,12} \node [fit=(prandifig6-\i-3)(prandifig6-\i-12), fill=black!12,minimum height=1.4cm,yshift=-.5ex] {};
\foreach \j in {8,10} \node [fit=(prandifig6-4-\j)(prandifig6-12-\j), fill=black!12,minimum width=1.26cm,inner xsep=0pt] {};
\foreach \x/\y in {8/8,8/10,10/8,10/10,12/8,12/10} \node [fit=(prandifig6-\x-\y), fill=black!50,minimum width=1.5cm,minimum height=1.4cm,yshift=-.75ex] {};
\end{scope}
\draw (prandifig6-4-12.south east) -| (prandifig6-12-4.south west);
\draw (prandifig6-3-12.south east) -| (prandifig6-12-3.south west);
\draw (prandifig6-3-11.north west) -- (prandifig6-3-12.north east);
\draw (prandifig6-3-9.north west) -- (prandifig6-3-10.north east);
\draw (prandifig6-3-5.north west) -- (prandifig6-3-8.north east);
\draw (prandifig6-11-3.north east) -- (prandifig6-12-3.north west);
\draw (prandifig6-9-3.north east) -- (prandifig6-10-3.north west);
\draw (prandifig6-5-3.north east) -- (prandifig6-8-3.north west);
\end{tikzpicture}}
\caption{Conflict matrix for \textsc{si} with \textsc{cai} (InterpretBank) TIC = 14.8\label{fig:prandi:6}\todo[inline]{ToDo for Langsci: Spacing between rows and columns, better rules!}}
\end{figure}
...
\pgfplotstableread{data/prandi15.csv}{\table}
    ...
    \pgfmathtruncatemacro\numberofcols{\pgfplotsretval-1}
        \begin{axis}[
            ...
            \end{axis}    
\end{tikzpicture}}
\caption{Overview of strategies used (all tools and stimuli)\todo[inline]{Todo for Langsci: fit to typeblock}\label{fig:prandi:15}}
\end{figure}
