...
After discussing the theoretical framework of our analysis (\sectref{sec:key:1}), we present our research desiderata and the structure of an exploratory study conducted to test our research methodology (\sectref{sec:key:2}). \sectref{sec:key:3} describes the rationale behind the stimuli used in the experiment and the features of the speeches used. We then present the results of the analysis of the transcriptions (\sectref{sec:key:4}), which we use to evaluate the appropriateness of our stimuli. In the conclusions, we address future work and provide suggestions for further research.
...
Not shown in the graphic representation of the model (\figref{fig:key:1}), but postulated by Wickens, is an additional pool of general capacity, which is always available to all tasks. In his adaptation of Wickens’s model to simultaneous interpreting, Seeber takes a step further, simplifying the graphical representation of Wickens’s model by turning it into a 2D model (see \figref{fig:key:2}). This has two main advantages. First, it allows seeing all “sides” of the cube (i.e. all dimensions) at once. Second, it graphically introduces the general capacity left out in Wickens’s “cube”. 
...
The footprint is integrated by a Conflict Matrix, which shows the degree of interference between two co-occurring tasks as the sum of the demand vectors for each sub-task and of the individual conflict coefficients between sub-tasks (see \figref{fig:key:3}). 
...
\begin{styleFramecontents}
\figref{fig:key:5}\textbf{:} Cognitive Resource Footprint for SI with a CAI tool/electronic glossary
\end{styleFramecontents}
...
Of the 6 terms placed at the end of the sentence, 3 should require a query in the glossary, because they are less frequent and thus probably unknown to the participants, and 3 should not.\footnote{This classification was based on the frequency of the terms as per the 2015 news corpus, the 2012 web corpus (UK) and the 2016 Wikipedia corpus for the English language (Projekt Deutscher Wortschatz, http://wortschatz.uni-leipzig.de).} The same is true for the terms placed in the middle of the sentence. Half of the stimuli in each speech should therefore require a query and half should not. This variable was introduced because we want to verify whether CAI tools, which are usually deemed to be user-friendlier and to take up fewer cognitive resources, allow participants to perform more queries without leading to a higher number of errors or omissions. \figref{fig:key:8} sums up the features of the stimuli and their distribution in each speech. For each speech, the stimuli can thus be classified according to their features, for future analysis. \tabref{tab:key:1} shows an example of this classification for the stimuli in speech nr. 1.
...
If we look at the strategies adopted when using different tools to look up terms in the glossary, we can notice that, when using InterpretBank (see \figref{fig:key:16}), a glossary query is the favorite strategy, except for one subject (the same as in the general analysis), who seems to resort mainly to approximation. The second most used strategies are omission and approximation, while the third most used strategies are world knowledge, approximation and omission.

Querying the glossary was the favorite strategy also when Excel (\figref{fig:key:17}) was used, in 4 cases out of 6, while the other two resorted, respectively, mainly to omission and paraphrase, and to world knowledge. There is not a clear preference as to the second most used strategy, while the third most used, in 4 cases out of 6, is world knowledge, followed by approximation and both omission and compression.
...
In the third case, in which the test subjects could look up terms in a Word glossary (see \figref{fig:key:18}), a glossary query also seems to be the favorite strategy, while paraphrasing is the second most used strategy in the sample. The third most used strategy is omission.
...
Fantinuoli, C. 2018. Computer-assisted interpreting: challenges and future perspectives. In: Isabel Durán and Gloria Corpas (eds.), Trends in e-tools and resources for translators and interpreters, 153-174. Leiden: Brill.

@misc{Gacek2015,
	author = {Gacek, Michael},
	title = {Softwarelösungen für {DolmetscherI}nnen ({MA} thesis). {{{U}}}niversity of Vienna.},
	year = {2015}
}
...
Gile, Daniel. 1997. Conference interpreting as a cognitive management problem. In Joseph H. Danks, Gregory M. Shreve, Stephen B. Fountain and Michael McBeath (Eds.). \textit{Cognitive Processes in Translation and Interpretation}. Sage, London, pages 196-214.

@misc{Gile1999,
	author = {Gile, Daniel},
	title = {Testing the Effort Models’ tightrope hypothesis in simultaneous interpreting - {{A}} contribution”. {{H}}ermes - Journal of Linguistics 23.},
	year = {1999}
}
...
@misc{Kahneman1973,
	...
	title = {Attention and Effort. {{P}}rentice-Hall, Englewood Cliffs, {NJ}.},
	year = {1973}
}
...
@article{Kantowitz1976,
	...
	journal = {Auditory secondary task, Acta Psychologica},
	pages = {343--362},
	title = {Testing tapping timesharing: 1},
	volume = {40},
	year = {1976}
}
...
@book{Prandi2015,
	address = {L'uso di InterpretBank nella didattica dell'interpretazione},
	author = {Prandi, Bianca.},
	publisher = {uno studio esplorativo (MA thesis). Università di Bologna},
	title = {\biberror{no title}},
	year = {2015a}
}
...
@book{Prandi2015,
	address = {The use of CAI tools in interpreters’ training},
	author = {Prandi, Bianca.},
	publisher = {a pilot study. In \textit{Proceedings of the Translating and the Computer 37 Conference}, London},
	title = {\biberror{no title}},
	year = {2015b}
}
...
@misc{Prandi2016,
	author = {Prandi, Bianca},
	title = {{Analysis of the impact of {CA}I tools on simultaneous interpreting with a focus on cognitive processes and terminology consistency. {{P}}oster presented at {TR}A\& {CO} Symposium, {Johannes} Gutenberg University Mainz/Germersheim.}},
	year = {2016}
}
...
@misc{Prandi2017,
	author = {Prandi, Bianca},
	title = {{Investigating cognitive load in simultaneous interpreting with the support of terminology management tools. {{P}}oster presented at 5\textsuperscript{th} Polish Eye Tracking Conference, Lublin.}},
	year = {2017a}
}
...
@misc{Prandi2017,
	author = {Prandi, B},
	title = {{Designing a Multimethod Study on the Use of {CA}I Tools during Simultaneous Interpreting. {{I}}n \textit{Proceedings of the Translating and the Computer 39 Conference in {London}}, {London}.}},
	year = {2017b}
}
...
Seeber, Kilian G. 2007. Thinking outside the cube: Modelling language processing tasks in a multiple resource paradigm. \textit{\citealt{Interspeech2007}}, Antwerp, pages 1382-1385.
...
@misc{Wadensjö1998,
	author = {Wadensjö, C},
	title = {Interpreting as interaction. {{L}}ondon/{New York}. {{L}}ongman.},
	year = {1998}
}
...
@article{Wickens1976,
	author = {Wickens,C. D},
	journal = {The effects of divided attention on information processing in tracking, Journal of Experimental Psychology: Human Perception and Performance},
	pages = {1--13},
	title = {\biberror{no title}},
	volume = {2},
	year = {1976}
...
Wickens, Christopher D. 1984. Processing resources in attention. In Raja Parasuraman and David R. Davies (Eds.). \textit{Varieties of attention}. Academic Press, New York, pages 63-102.

Wickens, Christopher D. 2002. Multiple resources and performance prediction. \textit{Theoretical issues in ergonomics science}, 3(2), pages 159-177.

@misc{Will2009,
	author = {Will, M},
	title = {Dolmetschorientierte Terminologiearbeit. {{M}}odell Und {Methode}. {{G}}unter Narr Verlag.},
	year = {2009}
}
...
@misc{Xu2015,
	author = {Xu, Ran},
	title = {Terminology Preparation for Simultaneous Interpreters ({PhD} thesis). {{{U}}}niversity of Leeds.},
	year = {2015}
}
